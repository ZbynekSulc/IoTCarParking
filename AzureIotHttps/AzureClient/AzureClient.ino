...
#include "Display.h"
// #include "OLE1D.h" // requires height 48 in Adafruit_SSD1306.h ...

const char* connectionString = "HostName=aershov.azure-devices.net;DeviceId=nodemcu1;SharedAccessKey=Ef4W5xqxPvScKO7WcptnXsep6UaG+1E5RVaD97A3cnY=";
const char* wifi_ssid = "Mira";
const char* wifi_pwd = "18022010";
const char* deviceLocation = "Brno";
...
/* 
 ...
 echo -n | openssl s_client -connect aershov.azure-devices.net:443 | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > cert.pem
...
IPAddress timeServer(216, 239, 35, 8); // timeServer(10, 13, 2, 239); // Update these with values suitable for your network.
...
//  powerPin.off();
  ...
  // led.on();
  Serial.println(hub.send(String(distance))); // response 204 means successful send of data to Azure IoT Hub
  // led.off();
